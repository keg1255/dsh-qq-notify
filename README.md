...
| `turn/end` 其他 kind（blocked/aborted/max-tokens/interrupted） | **立即推送**；未知 kind 静默忽略；**用户自己中止（`aborted` + 中止原因 `user`）不推送**——自己按的停止，不用再通知自己 | ⛔ 任务受阻（提示等待输入）/ ⏹ 已中止（含中止原因）/ 🔢 达到 token 上限 / ⏸ 已被打断 |
